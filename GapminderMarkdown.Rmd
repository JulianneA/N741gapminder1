---
title: "Selection of Descriptive Statistics for GapMinder Dataset"
author: "Julianne Ammirati (editing code by Melinda Higgins)"
...
---
#### The code used to generate this report is available [here](https://github.com/JulianneA/N741gapminder1)
## **Homework 01 SETUP**
...
```{r}
summary(gapminder) # Hashtag will give you comments
sd(gapminder$lifeExp) #request standard deviation of a particular variable
...
The mean life expectancy is `r round(mean(gapminder$lifeExp), digits=2)` years with a standard deviation of `r round(sd(gapminder$lifeExp), digits=2)` and a median of `r round(median(gapminder$lifeExp), digits=2)` (based on a sample size of n=`r round(length(gapminder$lifeExp), digits=0)`).
